			...
			<nav className="relative max-w-7xl w-full mx-auto px-4 sm:flex sm:items-center sm:justify-between sm:px-6 lg:px-8" aria-label="Global">
				...
				<div id="navbar-collapse-with-animation" className={`hs-collapse ${openMenu ? '' : 'hidden'} overflow-hidden transition-all duration-300 basis-full grow sm:block`}>
					<div className="flex flex-col gap-y-4 gap-x-0 mt-5 sm:flex-row sm:items-center sm:justify-end sm:gap-y-0 sm:gap-x-7 sm:mt-0 sm:pl-7">
						...
						<Link className={`${currentRoute === '/cvbuilder' ? "text-blue-600 dark:text-blue-500" : "text-gray-500 hover:text-gray-400 dark:text-gray-400 dark:hover:text-gray-500"} font-medium  sm:py-6 `} href="/cvbuilder" aria-current="page">Create CV</Link>
						<Link className={`${currentRoute === '/jobs' ? "text-blue-600 dark:text-blue-500" : "text-gray-500 hover:text-gray-400 dark:text-gray-400 dark:hover:text-gray-500"} font-medium  sm:py-6 `} href="/jobs" aria-current="page">Jobs</Link>
						<ThemeSwitcher />
